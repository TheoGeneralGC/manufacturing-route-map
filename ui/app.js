...
(() => {
  ...
  function exportCsv() {
    const fields = ['id','name','address','city','county','state','zip','latitude','longitude','industry','naics','employees','employee_range','employee_band','employee_scope','headcount_multiple_addresses','employee_source_address','owner_name','owner_role','contact_name','contact_role','phone','website','operating_status','evidence_type','geocode_quality','geocode_provider','source_name','source_url','updated_at','shortlisted','visited','visited_at','field_notes'];
    // Neutralize spreadsheet formula prefixes in any imported text, preserving the source in JSON.
    const cell = (value) => { let str = textValue(value); if (/^[\s]*[=+@-]/.test(str) && typeof value !== 'number') str = `'${str}`; return `"${str.replace(/"/g,'""')}"`; };
